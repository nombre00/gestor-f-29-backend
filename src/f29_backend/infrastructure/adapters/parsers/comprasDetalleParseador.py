...
def parse_detalle_compras(filename):
    # 1. Leemos el contenido completo del archivo como texto.
    with open(filename, 'r', encoding='utf-8') as f:
        lines = f.readlines()
    if not lines: # Si el archivo está vacío levanta un error.
        raise ValueError("El archivo está vacío")

    # 2. Modificamos solo la primera línea (el header), revisamos que no falten separadores, si falta, lo agregamos.
