...
class ResumenF29Repository:
    ...
    def find_by_cliente_periodo(self, cliente_id: int, periodo: str) -> Optional[ResumenF29]:
        """Busca el resumen F29 de un cliente para un período específico (formato YYYY-MM)"""
        return (
            self.db.query(ResumenF29)
            .filter(
    ...
    def find_by_usuario_y_mes(self, usuario_id: int, anio: int, mes: int) -> List[ResumenF29]:
        ...
        periodo es String(7) con formato YYYY-MM, se filtra por igualdad directa.
        """
        ...
        return (
            ...
            .filter(
                ...
                ResumenF29.periodo == periodo_str,      # ← fix: string directo
                Cliente.activo == True
            )
            .order_by(ResumenF29.periodo.desc())
            .all()
