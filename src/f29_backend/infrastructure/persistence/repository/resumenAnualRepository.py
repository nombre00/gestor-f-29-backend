...
class ResumenAnualRepository:
    ...
    # Busca un resumenAnual por un cliente durante un año.
    def get_by_cliente_and_año(self, cliente_id: int, año: str) -> Optional[ResumenAnual]:
        stmt = select(ResumenAnual).where(and_(ResumenAnual.cliente_id == cliente_id, ResumenAnual.año == año))
        return self.db.scalars(stmt).first()
    

    # Búsquedas de listas.
    # Busca todos los resumenes anuales de un cliente.
    def get_all_by_cliente(self, cliente_id: int) -> List[ResumenAnual]:
        stmt = (select(ResumenAnual).where(ResumenAnual.cliente_id == cliente_id).order_by(ResumenAnual.año.desc()))
        return self.db.scalars(stmt).all()
    
    # Busca todos los f29s de un cliente durante un año.
    def get_all_f29_del_año(self, cliente_id: int, año: str) -> List[ResumenF29]:
        stmt = (select(ResumenF29).where(and_(ResumenF29.cliente_id == cliente_id,ResumenF29.periodo.like(f"{año}-%"))
            ).order_by(ResumenF29.periodo.asc()))
        return self.db.scalars(stmt).all()

    
    # Create.
    def create(self,cliente_id: int,año: str,creado_por_usuario_id: int,detalles_json: dict,
        periodos_incluidos_json: List[str],estado: EstadoResumenAnual = EstadoResumenAnual.BORRADOR) -> ResumenAnual:
    ...
    # Actualizar.
    def update(self,resumen: ResumenAnual,detalles_json: Optional[dict] = None,
        periodos_incluidos_json: Optional[List[str]] = None,
        estado: Optional[EstadoResumenAnual] = None,) -> ResumenAnual:
        if resumen is None:
    ...
    # Busca periodos de una lista de f29s.
    ...
    # Cuenta los f29s de un cliente durante un año.
    def contar_f29s_del_año(self, cliente_id: int, año: str) -> int:
        stmt = select(func.count(ResumenF29.id)).where(and_(ResumenF29.cliente_id == cliente_id,ResumenF29.periodo.like(f"{año}-%")))
        return self.db.scalar(stmt) or 0   # más eficiente que len() + all()

    # Retorna un bool si el resumen anual existe o no para un cliente y un año.
    def existe_para_cliente_y_año(self, cliente_id: int, año: str) -> bool:
        stmt = select(func.count(ResumenAnual.id)).where(and_(ResumenAnual.cliente_id == cliente_id,ResumenAnual.año == año,))
