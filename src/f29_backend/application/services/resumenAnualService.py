...
class ResumenAnualService:
    ...
    def _acumular_f29s(self, f29s: List[ResumenF29]) -> ResumenF29:
        ...
        for f29 in f29s:
            ...
            # Remanente: solo el del último mes (el último sobreescribe)
            acum.remanente = f29.remanente

            # remuneraciones y honorarios: suma todos los valores numéricos
            for seccion in ['remuneraciones', 'honorarios']:
            ...
            sec_ppm = getattr(f29, 'ppm', {})
            if sec_ppm:
                target_ppm = acum.ppm
                if not target_ppm:
                    acum.ppm = sec_ppm.copy()
                else:
                    for k, v in sec_ppm.items():
                        if isinstance(v, (int, float)):
                            if k == 'tasa':
                                target_ppm[k] = v  # sobreescribe con el último mes
                            else:
